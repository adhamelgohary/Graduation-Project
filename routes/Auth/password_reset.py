...
try:
    ...
    mail = None
    print(
        "WARNING: Could not import 'mail' object from main app. Email sending will fail."
    )
...
def send_reset_email(user_email, token):
    ...
    if mail is None:
        current_app.logger.error(
            "Flask-Mail object ('mail') is not initialized. Cannot send email."
        )
        # Flash a generic error, don't reveal configuration issues to the user directly
        flash(
            "Password reset email could not be sent due to a server configuration issue. Please contact support.",
            "danger",
        )
        return False
    ...
    reset_url = url_for(
        "password_reset.reset_password_route", token=token, _external=True
    )
    ...
    try:
        ...
        msg = Message("Password Reset Request - Health Portal", recipients=[user_email])
...
"""
        ...
        current_app.logger.info(
            f"Password reset email successfully sent to {user_email}"
        )
        ...
        current_app.logger.error(
            f"Failed to send password reset email to {user_email}: {e}", exc_info=True
        )
        # Flash a generic error to the user
        flash(
            "An error occurred while trying to send the reset email. Please try again later or contact support.",
            "danger",
        )
...
def request_reset_route():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            cursor.execute(
                "SELECT user_id, email FROM users WHERE email = %s", (email,)
            )
            ...
            if user_exists:
                ...
                current_app.logger.info(
                    f"Reset token generated and stored for user_id {user['user_id']} ({email})."
                )
            ...
            if connection and connection.is_connected():
                try:
                    ...
                    pass  # Ignore rollback errors if connection already closed
            flash(
                "An error occurred processing your request. Please try again later.",
                "danger",
            )
            current_app.logger.error(
                f"Database error during password reset request for {email} (User ID: {user_id_log}): {db_err}"
            )
            return redirect(
                url_for("password_reset.request_reset_route")
            ...
            current_app.logger.error(
                f"Unexpected error during password reset request for {email} (User ID: {user_id_log}): {e}",
                exc_info=True,
            )
...
def reset_password_route(token):
    ...
    try:
        ...
        if not user:
            flash(
                "Invalid or expired password reset link. Please request a new one if needed.",
                "danger",
            )
            current_app.logger.warning(
                f"Invalid or expired token used for password reset attempt: {token}"
            )
            return redirect(url_for("login.login_route"))
        ...
        if request.method == "POST":
            ...
            if errors:
                ...
                return render_template(
                    "reset_password.html", token=token, form_data=request.form
                )
            ...
            try:
                ...
                if cursor.rowcount == 1:
                    ...
                    current_app.logger.info(
                        f"Password successfully reset for user_id {user['user_id']} using token {token[:6]}..."
                    )  # Log partial token
                    ...
                    connection.rollback()
                    flash(
                        "Could not reset password. The link might have expired or been used already. Please request a new link.",
                        "danger",
                    )
                    current_app.logger.error(
                        f"Failed to update password for user_id {user['user_id']} - rowcount was 0 (token mismatch/concurrency?). Token: {token[:6]}..."
                    )
                    return redirect(url_for("login.login_route"))
                ...
                if connection.is_connected():
                    try:
                        ...
                        pass
                flash(
                    "A database error occurred while updating your password. Please try again.",
                    "danger",
                )
                current_app.logger.error(
                    f"Database error during password update for user_id {user['user_id']}: {db_err_update}"
                )
                ...
                current_app.logger.error(
                    f"Unexpected error during password update for user_id {user['user_id']}: {e_update}",
                    exc_info=True,
                )
                return render_template(
                    "reset_password.html", token=token, form_data=request.form
    ...
    # --- Handle Errors During Initial Token Verification ---
        ...
        flash(
            "An error occurred verifying the reset link. Please try again later or contact support.",
            "danger",
        )
        current_app.logger.error(
            f"Database error checking reset token {token[:6]}...: {db_err_check}"
        )
        return redirect(url_for("login.login_route"))
        ...
        current_app.logger.error(
            f"Unexpected error checking reset token {token[:6]}...: {e_check}",
            exc_info=True,
        )
        return redirect(url_for("login.login_route"))
    finally:
