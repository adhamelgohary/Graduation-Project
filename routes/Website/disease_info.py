...
try:
    ...
    print(
        "CRITICAL ERROR: Failed to import get_relative_path_for_db in disease_info.py.",
        file=sys.stderr,
    )
...
def get_condition_details_for_website(condition_id):
    ...
    try:
        ...
        if not conn:
            getattr(logger, "error", print)(
                f"DB Connection failed for condition_details_for_website ID {condition_id}"
            )
            return None
        ...
        if not condition_data:
            getattr(logger, "warning", print)(
                f"Condition ID {condition_id} not found or not active for website."
            )
            return None
        ...
        if (
            img_file
            and current_app
            and "UPLOAD_FOLDER_CONDITIONS" in current_app.config
            and get_relative_path_for_db
        ):
            actual_filename = os.path.basename(img_file)
            full_image_path = os.path.join(
                current_app.config["UPLOAD_FOLDER_CONDITIONS"], actual_filename
            )
            relative_image_path = get_relative_path_for_db(full_image_path)
            if relative_image_path:
                condition_data["image_url"] = url_for(
                    "static", filename=relative_image_path
                )
            else:
                condition_data["image_url"] = url_for(
                    "static", filename="images/conditions/disease_placeholder.png"
                )
        else:
            condition_data["image_url"] = url_for(
                "static", filename="images/conditions/disease_placeholder.png"
        ...
        if dept_name == "Cardiology":
            condition_data["specific_css"] = "Website/Conditions/cardiodisease.css"
        elif dept_name == "Neurology":
            condition_data["specific_css"] = "Website/Conditions/neurodisease.css"
        elif dept_name == "Orthopedics":
            condition_data["specific_css"] = "Website/Conditions/orthodisease.css"
        elif dept_name == "Dermatology":
            condition_data["specific_css"] = "Website/Conditions/dermadisease.css"
        else:
            condition_data["specific_css"] = "Website/Conditions/generic_disease.css"

        condition_data["self_treatable_display"] = (
            "Yes"
            if condition_data.get("self_treatable")
            else ("No" if condition_data.get("self_treatable") is not None else "N/A")
        )
        ...
        getattr(logger, "error", print)(
            f"Error processing condition details for website ID {condition_id}: {e}",
            exc_info=True,
        )
        return None
    finally:
...
@disease_info_router.get("/<int:condition_id>")
def view_condition(condition_id):
    ...
    if not condition_details_wrapper or not condition_details_wrapper.get("condition"):
        raise HTTPException(
            status_code=404,
            detail=f"Information for condition ID {condition_id} not found.",
        )

    condition_data = condition_details_wrapper["condition"]
    logger = current_app.logger if current_app else print
    getattr(logger, "debug", print)(
        f"Viewing condition ID: {condition_id}, Name: {condition_data.get('name')}"
    )

    related_doctors = []
    department_id_for_condition = condition_data.get("department_id")
    condition_specialization_id = condition_data.get(
        "specialization_id"
    )  # Get condition's specialization ID

    getattr(logger, "debug", print)(
        f"Condition Dept ID: {department_id_for_condition}, Condition Spec ID: {condition_specialization_id}"
    )

    if (
        department_id_for_condition or condition_specialization_id
        ...
        try:
            ...
            if not conn_doc:
                getattr(logger, "error", print)(
                    "DB Connection failed for fetching doctors in view_condition."
                )
            else:
                ...
                if condition_specialization_id:
                    ...
                    getattr(logger, "info", print)(
                        f"Fetching doctors by condition's specialization_id: {condition_specialization_id}"
                    )
                elif (
                    department_id_for_condition
                ):  # Fallback to department if condition has no specific specialization
                    doctor_query_base += " AND d.department_id = %s"
                    query_params.append(department_id_for_condition)
                    getattr(logger, "info", print)(
                        f"Fetching doctors by condition's department_id (fallback): {department_id_for_condition}"
                    )
                ...
                if query_params:  # Only execute if we have criteria
                    ...
                    getattr(logger, "debug", print)(
                        f"Found {len(doctors_raw)} raw doctor records using query: {doctor_query_base} with params {query_params}"
                    )

                    for doc_idx, doc in enumerate(doctors_raw):
                        doc_user_id = doc.get("user_id", "N/A")
                        getattr(logger, "debug", print)(
                            f"Processing doctor #{doc_idx + 1} (User ID: {doc_user_id})"
                        )

                        path_relative_to_static_from_db = doc.get("profile_photo_url")
                        getattr(logger, "debug", print)(
                            f"  Doctor {doc_user_id} - path_relative_to_static_from_db: '{path_relative_to_static_from_db}'"
                        )

                        doc["profile_picture_url"] = url_for(
                            "static", filename="images/profile_pics/default_avatar.png"
                        )

                        if path_relative_to_static_from_db:
                            if current_app and current_app.static_folder:
                                full_abs_path_to_check = os.path.join(
                                    current_app.static_folder,
                                    path_relative_to_static_from_db,
                                )
                                if not os.path.exists(full_abs_path_to_check):
                                    getattr(logger, "warning", print)(
                                        f"  Doctor {doc_user_id} - Profile image file NOT FOUND at '{full_abs_path_to_check}' (DB path: '{path_relative_to_static_from_db}'). Using default."
                                    )
                                else:
                                    doc["profile_picture_url"] = url_for(
                                        "static",
                                        filename=path_relative_to_static_from_db,
                                    )
                                    getattr(logger, "info", print)(
                                        f"  Doctor {doc_user_id} - Successfully generated profile_picture_url: '{doc['profile_picture_url']}' using DB path."
                                    )
                            else:  # Should not happen if current_app exists
                                doc["profile_picture_url"] = url_for(
                                    "static", filename=path_relative_to_static_from_db
                                )
                                getattr(logger, "info", print)(
                                    f"  Doctor {doc_user_id} - Generated profile_picture_url (no existence check): '{doc['profile_picture_url']}' using DB path."
                                )
                        else:
                            getattr(logger, "warning", print)(
                                f"  Doctor {doc_user_id} - No profile picture path in DB (is None or empty). Using default."
                            )

                        bio = doc.get("biography")
                        doc["short_bio"] = (
                            (bio[:100] + "...") if bio and len(bio) > 100 else bio
                        )
                        related_doctors.append(doc)
                else:
                    getattr(logger, "info", print)(
                        "No department or specialization ID for condition to filter doctors."
                    )

        except Exception as e:
            getattr(logger, "error", print)(
                f"Error fetching/processing doctors for condition detail page (Dept {department_id_for_condition}, Spec {condition_specialization_id}): {e}",
                exc_info=True,
            )
        finally:
            if cursor_doc:
                cursor_doc.close()
            if conn_doc and conn_doc.is_connected():
                conn_doc.close()
    else:
        getattr(logger, "info", print)(
            "No department or specialization ID found for the condition, so no doctors will be fetched."
        )
